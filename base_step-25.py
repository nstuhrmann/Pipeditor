...
class ProcessingStep:
    ...
    #: Names a parameter may not use. Params is a dict, so a parameter
    #: called "keys" or "items" would be shadowed by the dict method and
    #: `self.p.keys` would silently return a bound method instead of the
    #: value. Caught at class-definition time rather than at runtime.
    _RESERVED_PARAM_NAMES = frozenset(
        n for n in dir(dict) if not n.startswith("__")) | {"p", "ctx", "inbox"}
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if clashes:
            raise TypeError(
                ...
                f"shadowed on self.p — rename them (e.g. 'keys' -> "
                f"'key_list').")
