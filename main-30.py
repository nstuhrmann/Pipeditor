...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self._btn_play.setToolTip("Play / pause frame-by-frame (F5)")
        self._btn_play.clicked.connect(self.toggle_playback)

        self._btn_step = QToolButton()
        self._btn_step.setText("Step")
        ...
        self._btn_stop.setToolTip("Stop playback and close the sequence "
                                  "(Shift+F5)")
    ...
    def _connect_scene(self, scene: PipelineScene):
        ...
        scene.edgeKindChanged.connect(self.on_edge_kind_changed)
    ...
    def on_edge_kind_changed(self, edge_item, kind: str):
        """Mirror a data/control switch into the model. Control edges are
        excluded from topological ordering, so this also decides whether
        the connection constrains execution order."""
        from_id = self.scene._find_node_id_for_port(edge_item.source_port)
        to_id = self.scene._find_node_id_for_port(edge_item.target_port)
        port = getattr(edge_item.target_port, "port_index", 0)
        if from_id and to_id:
            self.pipeline.set_edge_kind(from_id, to_id, port, kind)
            self.statusBar().showMessage(f"Edge set to {kind}", 3000)
    ...
    def _start_worker(self, frame_index: int, total: int):
        ...
        self._worker = PipelineWorker(self.pipeline, frame_index, total,
                                      in_sequence=self._playback_in_sequence)
    ...
    def _update_playback_buttons(self):
        ...
        sequence is open (begin_sequence outstanding)."""
        self._btn_play.setText("Pause" if self._playback_active else "Play")
    ...
    def _on_seq_finished(self, processed: int):
        ...
        tooltip_parts = []
        if (self._seq_worker.result.warnings if self._seq_worker.result else []):
            note += f"  ⚠ {len((self._seq_worker.result.warnings if self._seq_worker.result else []))} warning(s)."
            tooltip_parts.append(
                "Warnings:\n" + "\n".join((self._seq_worker.result.warnings if self._seq_worker.result else [])))
    ...
    def _on_run_finished(self, result):
        ...
        self.scene.update_side_channels(result)
