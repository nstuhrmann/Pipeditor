...
COLORS = {
    ...
    "header_step":         "#888888",
    # node outline. The unselected value matching "background" is what
    # makes the rim read as a gap between node and canvas — set it to
    # something else for a visible border.
    "node_outline":        "#222222",
    "port_outline":        "#222222",
    ...
    "port_out":            "#95c994",
    "port_control":        "#afca0b",
    # A port with nothing wired to it. Reads as an empty socket rather
    # than an error, because an unconnected OUTPUT is perfectly normal —
    # every chain ends in one.
    "port_unconnected":    "#3a3a39",
    # edges
    "edge":                "#dedfe0",
    "edge_selected":       "#e40521",
    "edge_control":        "#afca0b",
    "edge_temporary":      "#888888",   # while being dragged
    "edge_hover":          "#ffe48b",   # under the cursor
    # text on the canvas
    "text_params":         "#dddddd",
...
#: Set to [] to keep the system UI font, which is what the editor used
#: before this file existed — a bare QFont() with no family set.
FONT_FAMILIES = [
...
#: Point sizes, matching what the code used before these were collected
#: here. "node_title" was previously the application default size rather
#: than a fixed number — set it to 0 to go back to that.
FONT_SIZES = {
    "node_title":   0,      # 0 -> leave Qt's default point size alone
    "node_params":  7,
    "node_metric":  11,
    "port_label":   7,
    "side_channel": 7,
...
def font(role: str = "node_params", bold: bool = False) -> QFont:
    ...
    if family:
        f.setFamily(family)
    size = FONT_SIZES.get(role, 0)
    if size:                       # 0 keeps the application default
        f.setPointSize(size)
    f.setBold(bold)
    return f
