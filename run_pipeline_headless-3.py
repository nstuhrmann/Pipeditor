...
def main(path: str):
    ...
    # One path for any length. A single-frame pipeline is a sequence of
    # length 1, which matters: it is what opens the sinks, so an
    # image-in / image-out graph actually writes its output.
    ...
    result = pipeline.run_sequence(on_progress=on_progress)
    print(f"\ndone: {result.frames_processed}/{total} frame(s) processed")
    _print_timing(pipeline, f"run, {result.frames_processed} frame(s)",
                  use_mean=result.frames_processed > 1)
