...
def analyze_differences(ds: Dataset, out: Path) -> dict:
    ...
    plt.close(fig)
    return {f"{r['T']:g}C": {k: v for k, v in r.items() if k != "T"}
            for r in rows}
...
def make_plots(ds: Dataset, out: Path, pair_score, best_pair, pw_results,
    ...
    if vals.size:
        axs[0].set_ylim(0.5 * vals.min(), 2 * vals.max())    # sharey
...
def main(argv=None):
    ...
    for tint, ds in datasets.items():
        ...
        if args.diff_analysis:
            ...
            rep["difference_analysis"] = analyze_differences(ds, args.out)
