...
class NodeItem(QGraphicsObject):
    ...
    def boundingRect(self) -> QRectF:
        ...
        return QRectF(-self.LABEL_MARGIN, 0,
                      NODE_WIDTH + self.LABEL_MARGIN, self.height)
    ...
    def _add_port_label(self, text: str, y: float):
        """Input port labels live on the NODE (not the port) and sit just
        left of the box; boundingRect() above reserves room for them."""
        f = QFont(); f.setPointSize(7)
        shown = QFontMetrics(f).elidedText(
            text, Qt.ElideRight, self.LABEL_MARGIN - PORT_RADIUS - 6)
        lbl = QGraphicsTextItem(shown, self)
        lbl.setFont(f)
        lbl.setDefaultTextColor(QColor("#cccccc"))
        lbl.setPos(-(PORT_RADIUS + 4 + lbl.boundingRect().width()), y - 9)
        return lbl
    ...
    def contextMenuEvent(self, event):
        ...
        # through) or metrics (no single output port to pass through as).
        if self._kind() in ("source", "metric"):
            return
        menu = QMenu()
        menu.setStyleSheet(_DARK_MENU_STYLE)
...
class PipelineScene(QGraphicsScene):
    ...
    def update_previews(self, results: dict, metric_values: dict = None):
        ...
        for nid, item in self.node_items.items():
            if getattr(item.pipeline_node.step, "IS_METRIC", False):
                ...
                elif nid in results and not isinstance(results[nid],
                                                       np.ndarray):
                    # back-compat: older callers without metric_values
