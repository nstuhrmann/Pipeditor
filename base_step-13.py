...
class MetricCSVMixin:
    ...
    The class that declares the parameters also does the writing, so the
    CSV format can change without touching the executor.
    """
    CSV_PARAMS = [
    ...
    def write_csv(self, series: list, display_name: str) -> str:
        """Write [(frame_index, value)] for one batch.

        Returns "" if dumping is off or there is nothing to write, else
        the path; raises on I/O failure so the executor can turn it into
        an ordinary pipeline warning.
        """
        if not self.values.get("dump_csv") or not series:
            return ""
        path = (self.values.get("csv_path") or "").strip()
        if not path:
            path = display_name.replace(" ", "_") + ".csv"
        with open(path, "w", encoding="utf-8", newline="") as f:
            f.write("frame,value\n")
            for frame_index, value in series:
                f.write(f"{frame_index},{value}\n")
        return path
