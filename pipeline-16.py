...
class Pipeline:
    ...
    def _dump_metric_csvs(self, batch: RunResult):
        """Offer each metric its own per-frame series. Steps that don't
        implement write_csv (i.e. don't use MetricCSVMixin) simply don't
        get asked — the executor knows nothing about CSV parameters."""
        for nid, series in batch.metric_series.items():
            node = self.nodes.get(nid)
            writer = getattr(node.step, "write_csv", None) if node else None
            if writer is None:
                continue
            try:
                writer(series, node.display_name)
            except Exception as exc:
                batch.warnings.append(
                    f"'{node.display_name}': could not write CSV: {exc}")
