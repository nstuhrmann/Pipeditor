...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Parallel frame processing. Off by default: it only pays on
        # graphs with a large stateless share, and the serial path is
        # the reference implementation.
        self._parallel_workers = int(
            self._settings.value("parallel_workers", 1, type=int))
    ...
    def _build_menu(self):
        ...
        pm.addSeparator()
        self._act_parallel = QAction("Parallel Frame Processing", self,
                                     checkable=True)
        self._act_parallel.setChecked(self._parallel_workers > 1)
        self._act_parallel.setToolTip(
            "Process several frames concurrently during Process Sequence. "
            "No effect on Play or Preview.")
        self._act_parallel.toggled.connect(self._on_parallel_toggled)
        pm.addAction(self._act_parallel)
        self._add_action(pm, "Worker Count…", self.choose_worker_count)
    ...
    def _on_parallel_toggled(self, checked: bool):
        if checked and self._parallel_workers <= 1:
            self._parallel_workers = max(2, (os.cpu_count() or 4) // 2)
        elif not checked:
            self._parallel_workers = 1
        self._settings.setValue("parallel_workers", self._parallel_workers)
        self._report_parallel()

    def choose_worker_count(self):
        current = max(2, self._parallel_workers)
        count, ok = QInputDialog.getInt(
            self, "Worker Count",
            "Frames processed concurrently.\n\n"
            "Only nodes that are stateless can run several workers; "
            "sources, sinks, stateful steps and anything on a control "
            "edge stay serial. More workers than cores rarely helps.",
            current, 1, 64, 1)
        if not ok:
            return
        self._parallel_workers = count
        self._settings.setValue("parallel_workers", count)
        self._act_parallel.blockSignals(True)
        self._act_parallel.setChecked(count > 1)
        self._act_parallel.blockSignals(False)
        self._report_parallel()

    def _report_parallel(self):
        if self._parallel_workers > 1:
            self.statusBar().showMessage(
                f"Parallel processing on: {self._parallel_workers} workers "
                f"(Process Sequence only)", 5000)
        else:
            self.statusBar().showMessage("Parallel processing off", 4000)
    ...
    def process_full_sequence(self):
        ...
        # Playback and Preview stay serial deliberately: playback pulls
        # one frame per timer tick, so a pool has nothing to overlap, and
        # a preview is a single frame.
