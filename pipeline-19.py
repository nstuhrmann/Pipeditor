...
class Pipeline:
    ...
    def run_sequence(self, on_frame_done=None, on_progress=None,
        ...
        """Drain iter_sequence() into a single RunResult."""
        total = self.total_frames()
        batch = RunResult()
        seen_warnings: set = set()
        # No try/finally needed: iter_sequence() closes its own resources
        # in a finally, including when the generator is abandoned.
        for i, total, frame in self.iter_sequence(should_cancel):
