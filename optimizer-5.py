...
class ParameterOptimizer:
    ...
    def _local_search(self):
        ...
        try:
            ...
            # Budget exhausted mid-run, or scipy bailing out — whatever
            # was evaluated is already recorded in _best_x.
            pass
    ...
    def run(self, on_progress=None, should_cancel=None) -> OptimResult:
        ...
        try:
            if self.method == "local":
                ...
                # Random sampling needs exponentially many points as the
                # dimension grows, so above ~4 parameters it is nearly
                # useless — spend almost the whole budget refining from
                # the current values instead of scattering it.
                frac = 3 if len(self.targets) <= 4 else 12
                n_random = max(2, self.max_evals // frac)
