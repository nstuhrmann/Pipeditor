...
class PipelineWorker(QObject):
    ...
    finished = Signal(dict, list, dict)   # results, warnings, metric values
    failed   = Signal(str)
    ...
    def run(self):
        try:
            ...
            results = self.pipeline.run(
                ...
                metric_values_out=metric_values)
            self.finished.emit(results, warnings, metric_values)
...
class SequenceWorker(QObject):
    ...
    def __init__(self, pipeline: Pipeline):
        ...
        self.last_metric_values: dict = {}   # last frame's metric values
    ...
    def run(self):
        try:
            ...
            processed = self.pipeline.run_sequence(
                ...
                metric_values_out=self.last_metric_values)
