...
class ParamDialog(QDialog):
    ...
    def __init__(self, step, parent=None):
        ...
        for spec in step.PARAMS:
            ...
            # The hint goes on BOTH the control and its label: hovering
            # the word you are reading is the natural gesture, and a
            # tooltip only on the spin box is easy to never find.
            label_widget = QLabel(spec.label)
            if spec.hint:
                widget.setToolTip(spec.hint)
                label_widget.setToolTip(spec.hint)
            form.addRow(label_widget, widget)

        if not step.PARAMS:
            form.addRow(QLabel("This step has no parameters."))
    ...
    def _make_widget(self, spec, value):
        ...
        if spec.kind == "float":
            ...
            w.setDecimals(spec.display_decimals())
