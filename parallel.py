...
#: How many frames a mailbox may hold before its producers are held back.
DEFAULT_MAILBOX_DEPTH = 4
...
class _NodeManager:
    ...
    def mailbox_full(self) -> bool:
        return len(self.mailbox) >= _NodeManager.depth

    depth = DEFAULT_MAILBOX_DEPTH


class ParallelRunner:
    """Drives one committed run of a pipeline across a thread pool."""

    def __init__(self, pipeline, workers: int = 4,
                 mailbox_depth: int = DEFAULT_MAILBOX_DEPTH,
                 should_cancel=None):
        self.pipeline = pipeline
        self.workers = max(1, int(workers))
        self.should_cancel = should_cancel
        _NodeManager.depth = max(1, int(mailbox_depth))
    ...
    def _schedule(self, pool):
        ...
        for mgr in self.managers.values():
            if mgr.consumers and any(
                    c.mailbox_full() for c, _p, off in mgr.consumers
                    if off == 0):
...
def iter_sequence_parallel(pipeline, workers: int = 4,
                           mailbox_depth: int = DEFAULT_MAILBOX_DEPTH,
                           should_cancel=None):
