"""
Node editor graphics.

NodeItem      box for a ProcessingStep
PortItem      input (blue) or output (green) port circle
EdgeItem      bezier connection with optional image thumbnail
PipelineScene manages the graph visuals and syncs with pipeline data
"""
from PySide6.QtWidgets import (
    QGraphicsItem, QGraphicsObject, QGraphicsScene,
    QGraphicsPathItem, QGraphicsTextItem, QGraphicsEllipseItem,
    QGraphicsPixmapItem, QMenu, QGraphicsView, QApplication, QStyle,
)
from PySide6.QtGui import (
    QBrush, QColor, QPen, QPainterPath, QFont, QPixmap,
    QImage, QPainterPathStroker, QFontMetrics,
)
from PySide6.QtCore import Qt, QPointF, QRectF, Signal
from PySide6.QtGui import QTransform
import math

import numpy as np

from src.GUI.pipeline_editor import theme
from src.GUI.pipeline_editor.array_utils import METRIC_META_PREFIX
from src.GUI.pipeline_editor.base_step import step_source_location
from src.GUI.pipeline_editor.image_utils import numpy_to_thumbnail
from src.GUI.pipeline_editor.pipeline import EDGE_CONTROL, EDGE_DATA

NODE_WIDTH        = 170
NODE_HEADER_H     = 22
NODE_ROW_H        = 14
PORT_RADIUS       = 6
THUMB_W, THUMB_H  = 96, 72

# Header colours per node kind
# Edge routing styles, selectable from View > Edge Style. One global
# setting rather than per-edge: mixed routing in one graph reads as noise.
EDGE_STYLE_CURVED = "curved"
EDGE_STYLE_STRAIGHT = "straight"
EDGE_STYLE_ORTHOGONAL = "orthogonal"
EDGE_STYLE_ROUNDED = "rounded"
EDGE_STYLES = (EDGE_STYLE_CURVED, EDGE_STYLE_STRAIGHT,
               EDGE_STYLE_ORTHOGONAL, EDGE_STYLE_ROUNDED)

_HEADER = {
    "source": theme.color("header_source"),
    "sink":   theme.color("header_sink"),
    "metric": theme.color("header_metric"),
    "step":   theme.color("header_step"),
}

# Kept as module-level names because they are referenced all over this
# file; the VALUES all live in theme.py.
META_TEXT_COLOR = theme.COLORS["text_meta"]
CONTROL_TEXT_COLOR = theme.COLORS["text_control"]
CONTROL_EDGE_COLOR = theme.COLORS["edge_control"]
PORT_CONTROL_COLOR = theme.COLORS["port_control"]
PORT_UNCONNECTED_COLOR = theme.COLORS["port_unconnected"]
PORT_OUTLINE_COLOR = theme.COLORS["port_outline"]
PORT_IN_COLOR = theme.COLORS["port_in"]
PORT_OUT_COLOR = theme.COLORS["port_out"]
NODE_BODY_COLOR = theme.COLORS["node_body"]
NODE_OUTLINE = theme.COLORS["node_outline"]
NODE_OUTLINE_SELECTED = theme.COLORS["node_outline_selected"]
EDGE_HOVER_COLOR = theme.COLORS["edge_hover"]
EDGE_COLOR = theme.COLORS["edge"]
EDGE_SELECTED_COLOR = theme.COLORS["edge_selected"]
SCENE_BG_COLOR = theme.COLORS["background"]
# Port labels take their colour from the port itself (see
# PortItem.current_color), so the two never disagree.

# Shared dark stylesheet for right-click context menus in this module —
# without it, QMenu can end up with mismatched (e.g. black-on-black) text
# depending on the OS theme, since it otherwise inherits system defaults.
_DARK_MENU_STYLE = theme.menu_style()


def _numpy_to_pixmap(arr: np.ndarray) -> QPixmap:
    return numpy_to_thumbnail(arr, THUMB_W, THUMB_H)


# ---------------------------------------------------------------------------
# PortItem
# ---------------------------------------------------------------------------

_CLIPBOARD_MIME = "application/x-imgpipe-step"
#: Drag payload from the palette: just the step's class name.
PALETTE_MIME = "application/x-imgpipe-palette-step"


def clipboard_put_step(class_name: str, params: dict):
    """Put one step on the system clipboard as JSON under our own MIME
    type — so it survives between app instances, and cannot be confused
    with arbitrary copied text."""
    import json
    from PySide6.QtCore import QMimeData, QByteArray
    payload = json.dumps({"class_name": class_name, "params": params})
    mime = QMimeData()
    mime.setData(_CLIPBOARD_MIME, QByteArray(payload.encode("utf-8")))
    # Plain text too, so it can be pasted into a bug report or a chat.
    mime.setText(payload)
    QApplication.clipboard().setMimeData(mime)


def clipboard_step() -> dict:
    """The step on the clipboard, or {}."""
    import json
    data = QApplication.clipboard().mimeData()
    if data is None or not data.hasFormat(_CLIPBOARD_MIME):
        return {}
    try:
        return json.loads(bytes(data.data(_CLIPBOARD_MIME)).decode("utf-8"))
    except Exception:
        return {}


def _fmt_scalar(v) -> str:
    if isinstance(v, float):
        return f"{v:.4g}"
    if isinstance(v, (int, bool, str)):
        return str(v)
    return type(v).__name__


def format_bus_value(value) -> str:
    """Compact one-line rendering of a posted message, for the canvas."""
    if isinstance(value, dict):
        items = list(value.items())
        shown = ", ".join(f"{k}={_fmt_scalar(v)}" for k, v in items[:3])
        return "{" + shown + ("…" if len(items) > 3 else "") + "}"
    if isinstance(value, (list, tuple)):
        return "[" + ", ".join(_fmt_scalar(v) for v in value[:3]) + \
               ("…" if len(value) > 3 else "") + "]"
    return _fmt_scalar(value)


class PortItem(QGraphicsEllipseItem):
    """A connection point. Filled with its type colour once something is
    wired to it, and with port_unconnected while empty — `edges` is a
    plain list, so the two places that mutate it call
    refresh_connection_state() rather than the colour tracking itself."""

    def __init__(self, node_item: "NodeItem", is_output: bool,
                 port_index: int = 0, label: str = "", tooltip: str = "",
                 is_control: bool = False):
        super().__init__(-PORT_RADIUS, -PORT_RADIUS,
                         PORT_RADIUS * 2, PORT_RADIUS * 2)
        self.node_item = node_item
        self.is_output = is_output
        self.port_index = port_index
        self.is_control = is_control
        self.setPen(QPen(QColor(PORT_OUTLINE_COLOR), 1))
        self.setParentItem(node_item)
        self.setAcceptHoverEvents(True)
        self.edges: list["EdgeItem"] = []
        self.refresh_connection_state()
        if tooltip:
            self.setToolTip(tooltip)
        # NOTE: the port's text label is NOT created here. It sits to the
        # LEFT of the port, i.e. outside this item's 12px bounding rect —
        # Qt culls and misrepaints children drawn outside their parent's
        # bounds, which is why labels were invisible. NodeItem draws them
        # instead and widens its own boundingRect to cover them.

    def type_color(self) -> str:
        if self.is_control:
            return PORT_CONTROL_COLOR
        return PORT_OUT_COLOR if self.is_output else PORT_IN_COLOR

    def flag_when_unconnected(self) -> bool:
        """Whether leaving this port unwired means the graph is unfinished.

        Not every empty port is a problem, and colouring the harmless
        ones just trains you to ignore the colour:

          data input      always — the executor skips the node entirely
          data output     only on a step or a source, where nothing
                          downstream means the node does no work. A
                          metric is normally the end of a chain and a
                          sink has no output at all.
          control port    always — EMITS_CONTROL / ACCEPTS_CONTROL are
                          opt-in, so a node only has these if it intends
                          to use them. An unwired control port is exactly
                          the "loop that silently does nothing" case.
        """
        if self.is_control or not self.is_output:
            return True
        return self.node_item.pipeline_node.step.KIND in ("step", "source")

    def current_color(self) -> str:
        """The colour this port is actually painted right now."""
        unwired = not self.edges and self.flag_when_unconnected()
        return PORT_UNCONNECTED_COLOR if unwired else self.type_color()

    def refresh_connection_state(self):
        self.setBrush(QBrush(QColor(self.current_color())))
        self.update()
        # The label is painted by the NODE, so the node must repaint too.
        if self.node_item is not None:
            self.node_item.update()

    def scene_pos(self) -> QPointF:
        return self.mapToScene(0, 0)


# ---------------------------------------------------------------------------
# EdgeItem
# ---------------------------------------------------------------------------

class EdgeItem(QGraphicsPathItem):
    """An edge. kind="data" carries images; kind="control" carries values
    backwards to the target's NEXT frame and is drawn dashed, since it
    deliberately does not participate in execution order."""

    def __init__(self, source_port: PortItem,
                 target_port: "PortItem | None" = None,
                 show_preview: bool = False, kind: str = EDGE_DATA):
        super().__init__()
        self.source_port = source_port
        self.target_port = target_port
        self.kind = kind
        self._is_permanent = target_port is not None
        self.setZValue(-1)
        self.temp_end: QPointF | None = None

        # The thumbnail is intentionally NOT a child of this item. Qt groups
        # a child's stacking order with its parent's, so a thumbnail parented
        # to its edge could still end up rendered behind an unrelated edge
        # added later. Keeping it as an independent top-level scene item
        # with a high Z value guarantees previews always sit above every
        # connection line, regardless of add/draw order.
        self._thumb = QGraphicsPixmapItem()
        self._thumb.setZValue(10)
        self._thumb.setVisible(False)
        # Back-reference so PipelineScene can identify which edge (and
        # therefore which node's output) a double-clicked thumbnail
        # belongs to, without the thumbnail being a child item.
        self._thumb._owner_edge = self
        self._show_preview = False
        self._image_data: np.ndarray | None = None

        self._update_pen()
        if self._is_permanent:
            self.setFlag(QGraphicsItem.ItemIsSelectable, True)
            self.setAcceptHoverEvents(True)
        if show_preview:
            self.set_preview_visible(True)

    def _pen_for_kind(self) -> QPen:
        """Control edges are dashed amber: they carry values, not images,
        and are excluded from execution order, so they must not read as
        an ordinary connection."""
        if self.kind == EDGE_CONTROL:
            pen = QPen(QColor(CONTROL_EDGE_COLOR), 2, Qt.DashLine)
            pen.setDashPattern([5, 4])
            return pen
        return QPen(QColor(EDGE_COLOR), 2)

    def set_kind(self, kind: str):
        self.kind = kind
        if kind == EDGE_CONTROL:
            self.set_preview_visible(False)   # no image on a control edge
        self._update_pen()
        self.update()

    def _update_pen(self):
        if not self._is_permanent:
            self.setPen(QPen(theme.color("edge_temporary"), 1.5, Qt.DashLine))
        elif self.isSelected():
            self.setPen(QPen(QColor(EDGE_SELECTED_COLOR), 2.5))
        else:
            self.setPen(self._pen_for_kind())

    def paint(self, painter, option, widget=None):
        # Qt's default paint draws a dashed rubber-band rectangle around
        # a selected item. For an edge that box spans the whole bounding
        # rect of the curve — a huge rectangle across the canvas — and
        # says nothing the red pen doesn't already say. Clear the flag
        # and let the base class draw only the path.
        option.state &= ~QStyle.State_Selected
        super().paint(painter, option, widget)

    def shape(self):
        ps = QPainterPathStroker()
        ps.setWidth(12)
        return ps.createStroke(self.path())

    def itemChange(self, change, value):
        # HasChanged, not ItemSelectedChange: the latter fires BEFORE the
        # flag is updated, so isSelected() inside the handler still
        # reports the previous state and the pen ends up one selection
        # behind — the edge you just left stays red.
        if change == QGraphicsItem.ItemSelectedHasChanged:
            self._update_pen()
        return super().itemChange(change, value)

    def hoverEnterEvent(self, event):
        if not self.isSelected():
            self.setPen(QPen(QColor(EDGE_HOVER_COLOR), 2.5))
        super().hoverEnterEvent(event)

    def hoverLeaveEvent(self, event):
        self._update_pen()
        super().hoverLeaveEvent(event)

    # Routing style, shared by every edge (set from View > Edge Style).
    style = EDGE_STYLE_CURVED
    CORNER_RADIUS = 10

    def update_path(self):
        start = self.source_port.scene_pos()
        end = (self.target_port.scene_pos() if self.target_port
               else (self.temp_end or start))
        # Control ports sit on the BOTTOM of a node, so a control edge is
        # routed vertically and loops underneath — it reads as a feedback
        # loop instead of as another left-to-right connection.
        vertical = self.source_port.is_control or (
            self.target_port is not None and self.target_port.is_control)
        # Dispatch on the style NAME. An earlier version compared bound
        # methods with `is`, which is always False — every attribute
        # access creates a fresh bound-method object — so the rounded
        # flag never reached the router and Rounded drew as Orthogonal.
        if self.style == EDGE_STYLE_STRAIGHT:
            path = self._path_straight(start, end, vertical)
        elif self.style in (EDGE_STYLE_ORTHOGONAL, EDGE_STYLE_ROUNDED):
            path = self._path_orthogonal(
                start, end, vertical,
                rounded=(self.style == EDGE_STYLE_ROUNDED))
        else:
            path = self._path_curved(start, end, vertical)
        self.setPath(path)
        # The thumbnail is deliberately NOT a child of this item (see
        # __init__), so it does not inherit the new geometry — it has to
        # be moved explicitly whenever the path changes, which is what
        # makes it follow a node being dragged.
        self._update_pen()
        self._reposition_thumb()

    @staticmethod
    def _path_straight(start, end, vertical):
        path = QPainterPath(start)
        path.lineTo(end)
        return path

    @staticmethod
    def _path_curved(start, end, vertical):
        path = QPainterPath(start)
        if vertical:
            drop = max(abs(end.x() - start.x()) * 0.35, 45)
            drop = max(drop, abs(end.y() - start.y()) * 0.6 + 30)
            path.cubicTo(QPointF(start.x(), start.y() + drop),
                         QPointF(end.x(), end.y() + drop), end)
        else:
            dx = max(abs(end.x() - start.x()) * 0.5, 40)
            path.cubicTo(QPointF(start.x() + dx, start.y()),
                         QPointF(end.x() - dx, end.y()), end)
        return path

    def _path_orthogonal(self, start, end, vertical, rounded=False):
        """Right-angle routing. Data edges leave horizontally and turn at
        the midpoint; control edges drop below both nodes and come back
        up, so the loop never crosses the boxes it connects."""
        r = self.CORNER_RADIUS if rounded else 0
        if vertical:
            drop = max(abs(end.y() - start.y()) * 0.5, 0) + 40
            lane = max(start.y(), end.y()) + drop
            points = [start,
                      QPointF(start.x(), lane),
                      QPointF(end.x(), lane),
                      end]
        else:
            stub = 20
            mid = (start.x() + end.x()) * 0.5
            if end.x() - start.x() < 2 * stub:
                # Target is left of (or too close to) the source: go out,
                # down/up between the nodes, then back in.
                lane = (start.y() + end.y()) * 0.5
                points = [start,
                          QPointF(start.x() + stub, start.y()),
                          QPointF(start.x() + stub, lane),
                          QPointF(end.x() - stub, lane),
                          QPointF(end.x() - stub, end.y()),
                          end]
            else:
                points = [start,
                          QPointF(mid, start.y()),
                          QPointF(mid, end.y()),
                          end]
        return self._polyline(points, r)

    @staticmethod
    def _polyline(points, radius: float):
        """Join points with straight segments, optionally filleting each
        corner with a quadratic curve of the given radius."""
        path = QPainterPath(points[0])
        if radius <= 0:
            for p in points[1:]:
                path.lineTo(p)
            return path
        for i in range(1, len(points) - 1):
            prev, corner, nxt = points[i - 1], points[i], points[i + 1]
            v_in = corner - prev
            v_out = nxt - corner
            len_in = math.hypot(v_in.x(), v_in.y()) or 1.0
            len_out = math.hypot(v_out.x(), v_out.y()) or 1.0
            r = min(radius, len_in / 2, len_out / 2)
            path.lineTo(corner - v_in * (r / len_in))
            path.quadTo(corner, corner + v_out * (r / len_out))
        path.lineTo(points[-1])
        return path

    def _reposition_thumb(self):
        if not self._show_preview or not self.path():
            return
        mid = self.path().pointAtPercent(0.5)
        px = self._thumb.pixmap()
        self._thumb.setPos(mid.x() - px.width() / 2,
                           mid.y() - px.height() / 2)

    def set_image(self, arr: np.ndarray):
        self._image_data = arr
        if self._show_preview:
            self._refresh_thumb()

    def set_preview_visible(self, visible: bool):
        self._show_preview = visible
        if visible and self._image_data is not None:
            self._refresh_thumb()
        self._thumb.setVisible(visible and self._image_data is not None)

    def toggle_preview(self):
        self.set_preview_visible(not self._show_preview)

    def _refresh_thumb(self):
        if self._image_data is None:
            return
        px = _numpy_to_pixmap(self._image_data)
        self._thumb.setPixmap(px)
        self._thumb.setVisible(True)
        self._reposition_thumb()

    def add_to_scene(self, scene):
        """Add this edge and its (independent) thumbnail to the scene."""
        scene.addItem(self)
        scene.addItem(self._thumb)

    def remove_from_scene(self, scene):
        """Remove this edge and its thumbnail from the scene."""
        if self._thumb.scene() is not None:
            scene.removeItem(self._thumb)
        if self.scene() is not None:
            scene.removeItem(self)

    def contextMenuEvent(self, event):
        menu = QMenu()
        menu.setStyleSheet(_DARK_MENU_STYLE)
        is_control = self.kind == EDGE_CONTROL
        act_prev = None
        if not is_control:
            act_prev = menu.addAction(
                "Hide Preview" if self._show_preview else "Show Preview")
        act_rem = menu.addAction("Remove Connection")

        chosen = menu.exec(event.screenPos())
        scene = self.scene()
        if act_prev is not None and chosen == act_prev:
            self.toggle_preview()
        elif chosen == act_rem:
            if isinstance(scene, PipelineScene):
                scene.remove_edge_item_by_ref(self)


# ---------------------------------------------------------------------------
# NodeItem
# ---------------------------------------------------------------------------

class NodeItem(QGraphicsObject):
    doubleClicked      = Signal(object)
    bypassToggled      = Signal(object)   # pipeline_node
    deleteRequested    = Signal(object)   # pipeline_node
    duplicateRequested = Signal(object)   # pipeline_node

    BYPASSED_OPACITY = 0.45
    # Space reserved LEFT of the box for input port labels. It is part of
    # boundingRect() (so Qt paints the labels) but not of node_rect()
    # (so it isn't painted or clickable).
    LABEL_MARGIN = 74
    MSG_FONT_PT = 7
    MSG_ROW_H = 12          # one WRAPPED line, under the node

    def __init__(self, pipeline_node, label: str):
        super().__init__()
        step = pipeline_node.step
        self.pipeline_node = pipeline_node

        self.setFlag(QGraphicsItem.ItemIsMovable, True)
        self.setFlag(QGraphicsItem.ItemIsSelectable, True)
        self.setFlag(QGraphicsItem.ItemSendsGeometryChanges, True)

        # Hovering a node says what class it is and where that class is
        # defined — the fastest way from "this node misbehaves" to the
        # code, without hunting through the steps package.
        cls = type(pipeline_node.step)
        path, line = step_source_location(cls)
        self.setToolTip(f"{cls.__name__}  ({pipeline_node.step.KIND})"
                        + (f"\n{path}:{line}" if path else ""))

        is_source = step.KIND == "source"
        is_metric = step.KIND == "metric"
        n_inputs  = getattr(step, "N_INPUTS", 1)
        n_params  = len(step.PARAMS)

        body_rows = max(1, n_params)
        if is_metric:
            body_rows += 1   # extra row for the value display
        self.height = NODE_HEADER_H + body_rows * NODE_ROW_H + 10

        # ── Input ports ────────────────────────────────────────────────
        # Input 0 sits at the SAME height as the output port (header
        # center), so two adjacent modules connect with a straight
        # horizontal line instead of a slanted one. Additional inputs
        # (metric A/B etc.) are spaced below it.
        PORT_SPACING = 22
        self.input_ports: list[PortItem] = []
        # (text, y) for each input port label — painted directly in
        # paint() rather than as child QGraphicsTextItems. Child items
        # positioned outside the parent's bounds get culled by Qt, which
        # is why these labels never showed up.
        # (text, y, port) — the port so the label can take its
        # CURRENT colour, including the unconnected fill.
        self._input_labels: list = []
        # Bus messages this node emitted on the last executed frame,
        # already formatted. Shown under the box so a control loop is
        # visible in the graph instead of only in the code.
        self._side_lines: list = []      # [(text, colour)] before wrapping
        self._wrapped_lines: list = []   # [(text, colour)] after wrapping
        if not is_source:
            declared = tuple(getattr(step, "INPUT_LABELS", ()) or ())
            fallback = ["A", "B", "C", "D", "E", "F", "G", "H"]
            if n_inputs > 1:
                # make sure the box is tall enough for the lowest port
                need = int(NODE_HEADER_H / 2
                           + (n_inputs - 1) * PORT_SPACING
                           + PORT_RADIUS + 8)
                self.height = max(self.height, need)
            for i in range(n_inputs):
                name = declared[i] if i < len(declared) else ""
                # Show a label whenever the step named the port (even for
                # a single input) or whenever there's more than one port
                # to tell apart.
                shown = name or (fallback[i] if n_inputs > 1
                                 and i < len(fallback) else "")
                slot = (fallback[i] if i < len(fallback) else str(i))
                tip = (f"Input {slot}: {name}" if name
                       else f"Input {slot}" if n_inputs > 1 else "Input")
                y = NODE_HEADER_H / 2 + i * PORT_SPACING
                p = PortItem(self, is_output=False, port_index=i,
                             tooltip=tip)
                p.setPos(0, y)
                self.input_ports.append(p)
                if shown:
                    self._input_labels.append((shown, y, p))

        # ── Output port (hidden for metric nodes) ──────────────────────
        # ── Control ports ─────────────────────────────────────────────
        # Amber, on the BOTTOM edge, so a control link is visually a
        # different plane from the left-to-right data flow. A source has
        # no data inputs, so this is the only way to wire a feedback loop
        # back into one.
        step = pipeline_node.step
        self.control_in = None
        self.control_out = None
        if getattr(step, "ACCEPTS_CONTROL", False):
            self.control_in = PortItem(self, is_output=False, port_index=0,
                                       tooltip="Control in (values arrive "
                                               "on the next frame)",
                                       is_control=True)
            self.control_in.setPos(NODE_WIDTH * 0.28, self.height)
        if getattr(step, "EMITS_CONTROL", False):
            self.control_out = PortItem(self, is_output=True, port_index=0,
                                        tooltip="Control out (sent to the "
                                                "target's next frame)",
                                        is_control=True)
            self.control_out.setPos(NODE_WIDTH * 0.72, self.height)

        # ── Output port ───────────────────────────────────────────────
        # Metrics get one: they pass the image through unmodified, and
        # chaining metric -> metric -> Annotate is how several values
        # accumulate on one frame. Sinks do NOT: a sink is a terminus,
        # and an output port would invite wiring a processing step after
        # a writer, where the file already holds the pre-processed image.
        # To write the same frames twice, fan out from the node ABOVE the
        # writer — one output may feed any number of inputs.
        self.output_port: PortItem | None = None
        if pipeline_node.step.KIND != "sink":
            self.output_port = PortItem(self, is_output=True)
            self.output_port.setPos(NODE_WIDTH, NODE_HEADER_H / 2)

        # ── Labels ─────────────────────────────────────────────────────
        f = theme.font("node_title", bold=True)
        fm = QFontMetrics(f)
        # Elide titles that would overflow the node box; the full name
        # stays available as a tooltip on hover.
        elided = fm.elidedText(label, Qt.ElideRight, NODE_WIDTH - 16)
        self.title_item = QGraphicsTextItem(elided, self)
        # White on the lighter headers (grey, orange) is only ~2.8:1 —
        # picking by luminance keeps every header legible, and keeps
        # doing so if these colours are retuned later.
        self.title_item.setDefaultTextColor(
            theme.readable_on(_HEADER[self._kind()]))
        self.title_item.setFont(f)
        self.title_item.setPos(4, 0)
        if elided != label:
            self.setToolTip(label)

        self.params_text = QGraphicsTextItem(self._params_preview(), self)
        self.params_text.setDefaultTextColor(theme.color("text_params"))
        pf = theme.font("node_params")
        self.params_text.setFont(pf)
        self.params_text.setPos(4, NODE_HEADER_H - 2)

        # metric value display
        self._metric_text: QGraphicsTextItem | None = None
        if is_metric:
            self._metric_text = QGraphicsTextItem("—", self)
            self._metric_text.setDefaultTextColor(theme.color("text_metric_value"))
            f2 = theme.font("node_metric", bold=True)
            self._metric_text.setFont(f2)
            # Place it just below the params text's REAL extent (row
            # arithmetic broke when the param font/row height shrank),
            # and grow the box if the value line wouldn't fit.
            y_val = (self.params_text.pos().y()
                     + self.params_text.boundingRect().height() + 2)
            self._metric_text.setPos(8, y_val)
            needed = int(y_val
                         + self._metric_text.boundingRect().height() + 6)
            self.height = max(self.height, needed)

        x, y = pipeline_node.pos
        self.setPos(x, y)
        if pipeline_node.bypassed:
            self.setOpacity(self.BYPASSED_OPACITY)

    # ── bypass ─────────────────────────────────────────────────────────
    def set_bypassed(self, value: bool):
        self.pipeline_node.bypassed = value
        self.setOpacity(self.BYPASSED_OPACITY if value else 1.0)
        self.bypassToggled.emit(self.pipeline_node)

    # ── node kind ──────────────────────────────────────────────────────
    def _kind(self) -> str:
        """Header colour and port layout follow the step's own KIND.
        The keys of _HEADER are exactly ProcessingStep.KINDS."""
        return self.pipeline_node.step.KIND

    # ── labels ─────────────────────────────────────────────────────────
    def _params_preview(self) -> str:
        import os
        vals = self.pipeline_node.step.get_param_values()
        if not vals:
            return "(no parameters)"
        specs = {p.name: p for p in self.pipeline_node.step.PARAMS}
        lines = []
        for k, v in vals.items():
            # Same digit count the dialog edits with — a value shown as
            # 0.1 on the node but editable to four decimals reads as a
            # different number.
            if isinstance(v, float) and k in specs:
                v = f"{v:.{specs[k].display_decimals()}f}"
            elif isinstance(v, str) and len(v) > 22:
                # Strip the directory, then cap the basename too — a long
                # filename alone can still overflow the node box.
                v = os.path.basename(v)
                if len(v) > 20:
                    v = "…" + v[-19:]
                else:
                    v = "…" + v
            lines.append(f"{k}: {v}")
        return "\n".join(lines)

    def refresh_params_preview(self):
        self.params_text.setPlainText(self._params_preview())

    def set_metric_value(self, value):
        """None clears the display — a bypassed or skipped metric must
        not keep showing the last number it happened to produce."""
        if self._metric_text is None:
            return
        if value is None:
            self._metric_text.setPlainText("—")
        elif isinstance(value, float):
            self._metric_text.setPlainText(f"{value:.4g}")
        else:
            self._metric_text.setPlainText(str(value))

    # ── geometry ────────────────────────────────────────────────────────
    def all_ports(self) -> list:
        """Every port on this node, data and control. Anything that
        walks connections must use this — the previous code listed the
        data ports by hand, so control edges silently stopped following
        the node when it moved."""
        ports = list(self.input_ports)
        if self.output_port is not None:
            ports.append(self.output_port)
        if self.control_in is not None:
            ports.append(self.control_in)
        if self.control_out is not None:
            ports.append(self.control_out)
        return ports

    def node_rect(self) -> QRectF:
        """The visible box. Painting and hit-testing use THIS, not
        boundingRect() — which is deliberately wider (see below)."""
        return QRectF(0, 0, NODE_WIDTH, self.height)

    def boundingRect(self) -> QRectF:
        # Extends to the left of the box so the input port labels, which
        # are drawn outside it, are inside this item's declared area.
        # Without that Qt culls / fails to repaint them and they simply
        # never appear. The bottom is extended the same way for bus
        # messages.
        extra = (len(self._wrapped_lines) * self.MSG_ROW_H + 4
                 if self._wrapped_lines else 0)
        # HALO covers the outline stroked around the side-channel text and
        # the antialiasing on the node's own border. A boundingRect that
        # is even a pixel short leaves those pixels un-invalidated, which
        # is what draws streaks across the canvas when items move.
        halo = 3
        return QRectF(-self.LABEL_MARGIN - halo, -halo,
                      NODE_WIDTH + self.LABEL_MARGIN + 2 * halo,
                      self.height + extra + 2 * halo)

    @staticmethod
    def _wrap(text: str, fm: QFontMetrics, max_width: int) -> list:
        """Greedy word wrap to max_width. A single word too long to fit
        is elided rather than overflowing — message values can be one
        unbroken token."""
        words = text.split(" ")
        lines, current = [], ""
        for word in words:
            trial = word if not current else current + " " + word
            if fm.horizontalAdvance(trial) <= max_width:
                current = trial
                continue
            if current:
                lines.append(current)
            if fm.horizontalAdvance(word) <= max_width:
                current = word
            else:
                lines.append(fm.elidedText(word, Qt.ElideRight, max_width))
                current = ""
        if current:
            lines.append(current)
        return lines or [""]

    def set_side_channel_lines(self, entries: list):
        """Replace the side-channel text under this node.

        entries is [(text, colour)]. Text is wrapped to the width of the
        box, so the line COUNT (and thus boundingRect) depends on the
        content — Qt must be told before it re-reads the geometry, hence
        prepareGeometryChange()."""
        entries = list(entries)
        if entries == self._side_lines:
            return
        f = theme.font("side_channel")
        fm = QFontMetrics(f)
        wrapped = []
        for text, colour in entries:
            for piece in self._wrap(text, fm, NODE_WIDTH - 6):
                wrapped.append((piece, colour))
        self.prepareGeometryChange()
        self._side_lines = entries
        self._wrapped_lines = wrapped
        self.update()

    def _paint_side_channels(self, painter):
        if not self._wrapped_lines:
            return
        f = theme.font("side_channel")
        y = self.height + self.MSG_ROW_H - 2
        for shown, colour in self._wrapped_lines:
            path = QPainterPath()
            path.addText(3, y, f, shown)
            painter.setBrush(Qt.NoBrush)
            painter.setPen(QPen(theme.color("text_outline"), 3))
            painter.drawPath(path)                       # halo
            painter.setPen(Qt.NoPen)
            painter.setBrush(QBrush(QColor(colour)))
            painter.drawPath(path)
            y += self.MSG_ROW_H

    def shape(self):
        # Hit-testing must NOT include the label margin, or clicking in
        # the empty space left of a node would select and drag it.
        path = QPainterPath()
        path.addRoundedRect(self.node_rect(), 6, 6)
        return path

    def _paint_port_labels(self, painter):
        """Input port labels, drawn left of the box. Done with the
        painter (not child items) so they can't be culled, and elided to
        the reserved LABEL_MARGIN so they never run into the canvas.

        Drawn as an outlined glyph path rather than plain text: incoming
        edges are light grey and pass straight through this area, so flat
        text in a similar tone is unreadable where they cross. The dark
        halo keeps the label legible over an edge, the canvas, or a
        thumbnail; the fill matches the input-port blue so it reads as
        belonging to the port."""
        if not self._input_labels:
            return
        f = theme.font("port_label", bold=True)
        fm = QFontMetrics(f)
        avail = self.LABEL_MARGIN - PORT_RADIUS - 6
        for text, y, port in self._input_labels:
            shown = fm.elidedText(text, Qt.ElideRight, avail)
            w = fm.horizontalAdvance(shown)
            path = QPainterPath()
            path.addText(-(PORT_RADIUS + 5 + w), y + 4, f, shown)
            painter.setBrush(Qt.NoBrush)
            painter.setPen(QPen(theme.color("text_outline"), 3))
            painter.drawPath(path)                      # halo
            painter.setPen(Qt.NoPen)
            # Matches the port's own fill, unconnected state included —
            # label and port always read as one thing.
            painter.setBrush(QBrush(QColor(port.current_color())))
            painter.drawPath(path)                      # fill

    def paint(self, painter, option, widget=None):
        # 1. Body fill (no border yet)
        painter.setBrush(QBrush(QColor(NODE_BODY_COLOR)))
        painter.setPen(Qt.NoPen)
        painter.drawRoundedRect(self.node_rect(), 6, 6)

        # 2. Header fill
        painter.setBrush(QBrush(_HEADER[self._kind()]))
        painter.setPen(Qt.NoPen)
        painter.drawRoundedRect(QRectF(0, 0, NODE_WIDTH, NODE_HEADER_H), 6, 6)
        painter.drawRect(QRectF(0, NODE_HEADER_H - 6, NODE_WIDTH, 6))

        # 3. Outline drawn last, on top of both fills, so it's a uniform
        #    width all the way around instead of being partially covered
        #    by the header fill.
        painter.setBrush(Qt.NoBrush)
        painter.setPen(QPen(
            QColor(NODE_OUTLINE_SELECTED) if self.isSelected()
            else QColor(NODE_OUTLINE), 2))
        painter.drawRoundedRect(self.node_rect(), 6, 6)

        # 4. Input port labels, outside the box to the left.
        self._paint_port_labels(painter)

        # 5. Side channels this node fed, under the box.
        self._paint_side_channels(painter)

        # 5. Execution time, bottom-right corner (mean over the runs
        #    since the last reset — i.e. over the current batch).
        timing = self.pipeline_node.timing
        if timing.count:
            f = theme.font("timing")
            painter.setFont(f)
            painter.setPen(theme.color("text_timing"))
            text = f"{timing.mean_ms:.1f} ms"
            fm = QFontMetrics(f)
            painter.drawText(
                QPointF(NODE_WIDTH - 6 - fm.horizontalAdvance(text),
                        self.height - 4), text)

    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionChange:
            self.pipeline_node.pos = (value.x(), value.y())
        if change in (QGraphicsItem.ItemPositionChange,
                      QGraphicsItem.ItemPositionHasChanged):
            for port in self.all_ports():
                for edge in port.edges:
                    edge.update_path()
        return super().itemChange(change, value)

    def mouseDoubleClickEvent(self, event):
        self.doubleClicked.emit(self.pipeline_node)
        super().mouseDoubleClickEvent(event)

    def contextMenuEvent(self, event):
        # Select what you right-clicked, so the menu visibly applies to
        # THIS node — otherwise the highlight can sit on a different node
        # while you pick Delete.
        if not self.isSelected():
            scene = self.scene()
            if scene is not None:
                scene.clearSelection()
            self.setSelected(True)

        menu = QMenu()
        menu.setStyleSheet(_DARK_MENU_STYLE)
        # Bypass isn't meaningful for a source — nothing upstream to
        # pass through. Everything else, metrics included, can be
        # switched out while leaving the graph connected.
        act_bypass = None
        if self._kind() != "source":
            act_bypass = menu.addAction("Bypass")
            act_bypass.setCheckable(True)
            act_bypass.setChecked(self.pipeline_node.bypassed)
            menu.addSeparator()
        act_params = menu.addAction("Parameter View…")
        act_output = menu.addAction("Open Output Preview")
        menu.addSeparator()
        act_copy = menu.addAction("Copy Step")
        act_src = menu.addAction("Copy Source Location")
        path, line = step_source_location(type(self.pipeline_node.step))
        act_src.setEnabled(bool(path))
        if path:
            act_src.setToolTip(f"{path}:{line}")
        menu.addSeparator()
        act_dup = menu.addAction("Duplicate")
        act_delete = menu.addAction("Delete")
        chosen = menu.exec(event.screenPos())
        scene = self.scene()
        if act_bypass is not None and chosen == act_bypass:
            self.set_bypassed(not self.pipeline_node.bypassed)
        elif chosen == act_params:
            # Same path as double-clicking, which is otherwise the only
            # way in and easy not to discover.
            self.doubleClicked.emit(self.pipeline_node)
        elif chosen == act_output and isinstance(scene, PipelineScene):
            # The rendered frame, not the settings — the window that
            # updates live while a sequence plays.
            scene.outputPreviewRequested.emit(self.pipeline_node)
        elif chosen == act_copy and isinstance(scene, PipelineScene):
            scene.nodeCopyRequested.emit(self.pipeline_node)
        elif chosen == act_src and path:
            # "file:line" is what an editor's go-to-file accepts and what
            # a traceback prints, so it pastes straight into either.
            location = f"{path}:{line}"
            QApplication.clipboard().setText(location)
            if isinstance(scene, PipelineScene):
                scene.statusMessage.emit(f"Copied: {location}")
        elif chosen == act_dup:
            self.duplicateRequested.emit(self.pipeline_node)
        elif chosen == act_delete:
            # Deletion is owned by MainWindow (it also closes the node's
            # preview window and updates the model) — just request it.
            self.deleteRequested.emit(self.pipeline_node)


# ---------------------------------------------------------------------------
# PipelineScene
# ---------------------------------------------------------------------------

class PipelineView(QGraphicsView):
    """QGraphicsView for the node editor with middle-mouse-drag panning.
    Left-drag stays rubber-band selection / node dragging, so panning
    gets its own button rather than overloading an existing gesture."""

    ZOOM_STEP = 1.15
    ZOOM_MIN = 0.15
    ZOOM_MAX = 6.0

    def __init__(self, scene=None, parent=None):
        super().__init__(scene, parent)
        self._panning = False
        self._pan_last = None
        self._zoom = 1.0
        # Zoom toward the cursor rather than the view centre — otherwise
        # zooming in on a node in the corner walks it off screen and you
        # pan back after every step.
        self.setTransformationAnchor(QGraphicsView.AnchorUnderMouse)
        self.setAcceptDrops(True)

    def wheelEvent(self, event):
        factor = (self.ZOOM_STEP if event.angleDelta().y() > 0
                  else 1.0 / self.ZOOM_STEP)
        self.zoom_by(factor)

    def zoom_by(self, factor: float):
        target = self._zoom * factor
        if not self.ZOOM_MIN <= target <= self.ZOOM_MAX:
            return
        self._zoom = target
        self.scale(factor, factor)

    def reset_zoom(self):
        self.scale(1.0 / self._zoom, 1.0 / self._zoom)
        self._zoom = 1.0

    def fit_to_content(self):
        rect = self.scene().itemsBoundingRect() if self.scene() else None
        if rect is None or rect.isEmpty():
            return
        self.fitInView(rect.adjusted(-40, -40, 40, 40), Qt.KeepAspectRatio)
        # fitInView sets the transform directly, so recover the resulting
        # scale instead of assuming it — otherwise the next wheel step
        # would apply the wrong bounds.
        self._zoom = self.transform().m11()

    # --- drag & drop from the palette ------------------------------------
    def dragEnterEvent(self, event):
        if event.mimeData().hasFormat(PALETTE_MIME):
            event.acceptProposedAction()
        else:
            super().dragEnterEvent(event)

    def dragMoveEvent(self, event):
        if event.mimeData().hasFormat(PALETTE_MIME):
            event.acceptProposedAction()
        else:
            super().dragMoveEvent(event)

    def dropEvent(self, event):
        data = event.mimeData()
        if not data.hasFormat(PALETTE_MIME):
            super().dropEvent(event)
            return
        class_name = bytes(data.data(PALETTE_MIME)).decode("utf-8")
        scene = self.scene()
        if isinstance(scene, PipelineScene):
            # Drops land where the cursor is, which is the whole point of
            # dragging rather than double-clicking the palette entry.
            scene.addStepRequested.emit(class_name,
                                        self.mapToScene(event.position().toPoint()))
        event.acceptProposedAction()

    def mousePressEvent(self, event):
        if event.button() == Qt.MiddleButton:
            self._panning = True
            self._pan_last = event.position()
            self.setCursor(Qt.ClosedHandCursor)
            event.accept()
            return
        super().mousePressEvent(event)

    def mouseMoveEvent(self, event):
        if self._panning:
            delta = event.position() - self._pan_last
            self._pan_last = event.position()
            self.horizontalScrollBar().setValue(
                int(self.horizontalScrollBar().value() - delta.x()))
            self.verticalScrollBar().setValue(
                int(self.verticalScrollBar().value() - delta.y()))
            event.accept()
            return
        super().mouseMoveEvent(event)

    def mouseReleaseEvent(self, event):
        if event.button() == Qt.MiddleButton and self._panning:
            self._panning = False
            self.setCursor(Qt.ArrowCursor)
            event.accept()
            return
        super().mouseReleaseEvent(event)


class PipelineScene(QGraphicsScene):
    nodeDoubleClicked      = Signal(object)
    graphChanged           = Signal()
    edgeRequested          = Signal(str, str, int, str)  # from, to, port, kind
    edgeRemoved            = Signal(str, str, int)   # from_id, to_id, to_port
    thumbnailDoubleClicked = Signal(object)          # EdgeItem
    nodeBypassToggled      = Signal(object)          # PipelineNode
    nodeDeleteRequested    = Signal(object)          # PipelineNode
    nodeDuplicateRequested = Signal(object)          # PipelineNode
    statusMessage          = Signal(str)             # transient status text
    nodeCopyRequested      = Signal(object)          # PipelineNode
    outputPreviewRequested = Signal(object)          # PipelineNode
    fitRequested           = Signal()                # fit view to content
    pasteRequested         = Signal(object)          # QPointF (scene pos)
    addStepRequested       = Signal(str, object)     # class_name, QPointF
    edgeStyleRequested     = Signal(str)             # style name

    def __init__(self, pipeline):
        super().__init__()
        self.pipeline = pipeline
        self.setSceneRect(-2000, -2000, 4000, 4000)
        self.setBackgroundBrush(QBrush(QColor(SCENE_BG_COLOR)))
        self.node_items: dict[str, NodeItem] = {}
        self.edge_items: list[EdgeItem] = []
        # Side-channel display toggles (View menu).
        self.show_meta = True
        self.show_control = True
        self._drag_edge: EdgeItem | None = None
        self._drag_source_port: PortItem | None = None

    # ── nodes ──────────────────────────────────────────────────────────

    def add_node_item(self, pipeline_node) -> NodeItem:
        item = NodeItem(pipeline_node, pipeline_node.display_name)
        item.doubleClicked.connect(self.nodeDoubleClicked.emit)
        item.bypassToggled.connect(self.nodeBypassToggled.emit)
        item.deleteRequested.connect(self.nodeDeleteRequested.emit)
        item.duplicateRequested.connect(self.nodeDuplicateRequested.emit)
        self.addItem(item)
        self.node_items[pipeline_node.id] = item
        return item

    def remove_node_item(self, node_id: str):
        item = self.node_items.pop(node_id, None)
        if not item:
            return
        # all_ports() includes the control ports — building this list by
        # hand is what left control edges orphaned in the scene after a
        # node was deleted or the pipeline replaced.
        for port in item.all_ports():
            for edge in list(port.edges):
                self._remove_edge_item(edge)
        self.removeItem(item)

    # ── edges ──────────────────────────────────────────────────────────

    def add_edge_item(self, from_id: str, to_id: str,
                      to_port: int = 0, show_preview: bool = False,
                      kind: str = EDGE_DATA):
        src_item = self.node_items[from_id]
        dst_item = self.node_items[to_id]
        if kind == EDGE_CONTROL:
            src_port = src_item.control_out
            dst_port = dst_item.control_in
        else:
            src_port = src_item.output_port
            dst_port = (dst_item.input_ports[to_port]
                        if to_port < len(dst_item.input_ports) else None)
        if src_port is None or dst_port is None:
            return

        # clear old edge to this specific input port
        for old in list(dst_port.edges):
            self._remove_edge_item(old)

        edge = EdgeItem(src_port, dst_port,
                        show_preview=show_preview, kind=kind)
        src_port.edges.append(edge)
        dst_port.edges.append(edge)
        src_port.refresh_connection_state()
        dst_port.refresh_connection_state()
        edge.add_to_scene(self)
        self.edge_items.append(edge)
        edge.update_path()
        return edge

    def _remove_edge_item(self, edge: EdgeItem):
        if edge.source_port:
            edge.source_port.edges = [e for e in edge.source_port.edges
                                      if e is not edge]
        if edge.target_port:
            edge.target_port.edges = [e for e in edge.target_port.edges
                                      if e is not edge]
        for port in (edge.source_port, edge.target_port):
            if port is not None:
                port.refresh_connection_state()
        if edge in self.edge_items:
            self.edge_items.remove(edge)
        edge.remove_from_scene(self)

    def remove_edge_item_by_ref(self, edge: EdgeItem):
        from_id = self._find_node_id_for_port(edge.source_port)
        to_id   = self._find_node_id_for_port(edge.target_port)
        to_port = edge.target_port.port_index if edge.target_port else 0
        self._remove_edge_item(edge)
        if from_id and to_id:
            self.edgeRemoved.emit(from_id, to_id, to_port)

    # ── previews / metric values ───────────────────────────────────────

    def set_edge_style(self, style: str):
        EdgeItem.style = style
        for edge in self.edge_items:
            edge.update_path()

    def contextMenuEvent(self, event):
        """Right-click on empty canvas. Qt only reaches the scene when no
        item accepted the event, so this cannot shadow the node or edge
        menus."""
        if self.itemAt(event.scenePos(), QTransform()) is not None:
            super().contextMenuEvent(event)
            return
        pos = event.scenePos()
        menu = QMenu()
        menu.setStyleSheet(_DARK_MENU_STYLE)

        add_menu = menu.addMenu("Add Step")
        self._build_add_step_menu(add_menu, pos)

        act_paste = menu.addAction("Paste Step")
        act_paste.setEnabled(bool(clipboard_step()))
        menu.addSeparator()
        act_all = menu.addAction("Select All")
        act_fit = menu.addAction("Fit to View")
        style_menu = menu.addMenu("Edge Style")
        style_actions = {}
        for style in EDGE_STYLES:
            act = style_menu.addAction(style.title())
            act.setCheckable(True)
            act.setChecked(EdgeItem.style == style)
            style_actions[act] = style

        chosen = menu.exec(event.screenPos())
        if chosen == act_paste:
            self.pasteRequested.emit(pos)
        elif chosen == act_all:
            for item in self.node_items.values():
                item.setSelected(True)
        elif chosen == act_fit:
            self.fitRequested.emit()
        elif chosen in style_actions:
            self.edgeStyleRequested.emit(style_actions[chosen])

    def _build_add_step_menu(self, menu, pos):
        """Mirror the palette's category hierarchy, so "Filter/Denoise"
        nests the same way in both places."""
        from src.GUI.pipeline_editor.base_step import STEP_REGISTRY
        tree: dict = {}
        for class_name, cls in sorted(STEP_REGISTRY.items(),
                                      key=lambda kv: kv[1].NAME):
            node = tree
            for part in [p.strip() for p in cls.CATEGORY.split("/")
                         if p.strip()] or ["General"]:
                node = node.setdefault(part, {})
            node.setdefault("__steps__", []).append((cls.NAME, class_name))

        def build(parent_menu, subtree):
            for key in sorted(k for k in subtree if k != "__steps__"):
                build(parent_menu.addMenu(key), subtree[key])
            for name, class_name in subtree.get("__steps__", []):
                act = parent_menu.addAction(name)
                act.triggered.connect(
                    lambda _c=False, cn=class_name:
                    self.addStepRequested.emit(cn, pos))
        build(menu, tree)

    def update_side_channels(self):
        """Show what each node put on the non-image channels for the
        frame just executed.

        Teal  = metadata it emitted itself (forward, riding with the
                frame). Only its OWN contribution — showing everything
                inherited from upstream would repeat the same keys on
                every node down the chain.
        Amber = control values it sent (backward, arriving at the target
                on the next frame), matching the control ports and edges.
        """
        for nid, item in self.node_items.items():
            node = self.pipeline.nodes.get(nid)
            entries = []
            if node is not None and (self.show_meta or self.show_control):
                emitted = ((getattr(node, "last_emitted", None) or {})
                           if self.show_meta else {})
                for key, value in emitted.items():
                    # A metric's key is "metric:<its own display name>",
                    # which under that very node would just repeat the
                    # title — show it as plain "metric".
                    label = ("metric" if key.startswith(METRIC_META_PREFIX)
                             else key)
                    entries.append((f"{label}: {format_bus_value(value)}",
                                    META_TEXT_COLOR))
                sent = ((getattr(node, "last_control", None) or {})
                        if self.show_control else {})
                if sent:
                    body = ", ".join(f"{k}={format_bus_value(v)}"
                                     for k, v in sent.items())
                    entries.append((f"ctrl -> {body}", CONTROL_TEXT_COLOR))
            item.set_side_channel_lines(entries)

    def update_previews(self, results: dict, metric_values: dict):
        """Update edge thumbnails and metric value labels after a run.
        `results` holds images for every node (metrics included — theirs
        is input A with the value overlaid); the raw metric values come
        separately in `metric_values` (node_id -> float/str)."""
        target_ids = {e[1] for e in self.pipeline.edges}
        source_ids = {e[0] for e in self.pipeline.edges}
        root_ids     = {nid for nid in self.pipeline.nodes
                        if nid not in target_ids}
        terminal_ids = {nid for nid in self.pipeline.nodes
                        if nid not in source_ids}

        # A node whose output fans out to several downstream nodes would
        # otherwise get an identical thumbnail on every outgoing edge.
        # Auto-show at most one preview per SOURCE node — and seed the
        # set with sources that already have a preview visible, so a
        # manually enabled one counts and doesn't get a duplicate added
        # next to it.
        previewed_sources = {
            self._find_node_id_for_port(e.source_port)
            for e in self.edge_items
            if getattr(e, "_show_preview", False) and e.source_port is not None
        }

        for edge in self.edge_items:
            if not edge._is_permanent or edge.source_port is None:
                continue
            from_id = self._find_node_id_for_port(edge.source_port)
            to_id   = self._find_node_id_for_port(edge.target_port)
            if from_id and from_id in results:
                val = results[from_id]
                if isinstance(val, np.ndarray):
                    # Every edge still gets the image, so a preview the
                    # user switched on by hand keeps updating.
                    edge.set_image(val)
                    is_first = from_id in root_ids
                    is_last  = to_id in terminal_ids
                    if ((is_first or is_last)
                            and from_id not in previewed_sources):
                        edge.set_preview_visible(True)
                        previewed_sources.add(from_id)

        # repaint all nodes so the timing corner text refreshes
        for item in self.node_items.values():
            item.update()

        # update metric node labels
        for nid, item in self.node_items.items():
            if item.pipeline_node.step.KIND == "metric":
                if metric_values and nid in metric_values:
                    item.set_metric_value(metric_values[nid])
                else:
                    # Bypassed (or skipped): clear, so a stale number
                    # can't be mistaken for a live measurement.
                    item.set_metric_value(None)

    # ── keyboard ────────────────────────────────────────────────────────

    def keyPressEvent(self, event):
        if event.key() in (Qt.Key_Delete, Qt.Key_Backspace):
            for item in list(self.selectedItems()):
                if isinstance(item, EdgeItem) and item._is_permanent:
                    self.remove_edge_item_by_ref(item)
        super().keyPressEvent(event)

    # ── mouse: draw connections ─────────────────────────────────────────

    def mousePressEvent(self, event):
        view = self.views()[0] if self.views() else None
        item = self.itemAt(event.scenePos(), view.transform()) if view else None
        if isinstance(item, PortItem) and item.is_output:
            self._drag_source_port = item
            self._drag_edge = EdgeItem(item)
            self._drag_edge.temp_end = event.scenePos()
            self._drag_edge.add_to_scene(self)
            self._drag_edge.update_path()
            return
        super().mousePressEvent(event)

    def mouseDoubleClickEvent(self, event):
        view = self.views()[0] if self.views() else None
        item = self.itemAt(event.scenePos(), view.transform()) if view else None
        # Thumbnails are independent top-level items (see EdgeItem), so a
        # double-click on one won't reach NodeItem/EdgeItem handlers on
        # its own — catch it here via the back-reference we tagged it with.
        owner_edge = getattr(item, "_owner_edge", None)
        if owner_edge is not None:
            self.thumbnailDoubleClicked.emit(owner_edge)
            return
        super().mouseDoubleClickEvent(event)

    def mouseMoveEvent(self, event):
        if self._drag_edge:
            self._drag_edge.temp_end = event.scenePos()
            self._drag_edge.update_path()
            return
        super().mouseMoveEvent(event)

    def mouseReleaseEvent(self, event):
        if self._drag_edge:
            view = self.views()[0] if self.views() else None
            target = self.itemAt(event.scenePos(),
                                 view.transform()) if view else None
            self._drag_edge.remove_from_scene(self)
            self._drag_edge = None
            src_port = self._drag_source_port
            self._drag_source_port = None
            if (isinstance(target, PortItem) and not target.is_output
                    and src_port):
                from_id = self._find_node_id_for_port(src_port)
                to_id   = self._find_node_id_for_port(target)
                # Control and data ports don't interconnect: the two
                # carry different things, and silently coercing one into
                # the other is how you get a loop that mysteriously does
                # nothing.
                if src_port.is_control != target.is_control:
                    return
                kind = EDGE_CONTROL if target.is_control else EDGE_DATA
                if from_id and to_id and (from_id != to_id
                                          or kind == EDGE_CONTROL):
                    self.edgeRequested.emit(from_id, to_id,
                                            target.port_index, kind)
            return
        super().mouseReleaseEvent(event)

    def _find_node_id_for_port(self, port):
        if port is None:
            return None
        for nid, item in self.node_items.items():
            if port in item.all_ports():
                return nid
        return None
