...
class Frame(np.ndarray):
    """An ndarray that carries per-frame metadata in `.meta`.

    Used ONLY at the executor<->step boundary: the executor hands steps
    wrapped inputs so they can read `image.meta[...]`, and re-wraps the
    output with the merged metadata. It deliberately does NOT rely on
    the subclass surviving inside a step — OpenCV strips subclasses, and
    almost every step calls cv2, so propagation is the executor's job.
    That also means a step needs no awareness of metadata for metadata
    to flow through it.

    Writes go through ProcessingStep.emit_meta(), not `image.meta[...] =`,
    because a step normally returns a fresh array from cv2 that has no
    `.meta` at all — making steps responsible for re-attaching would
    silently lose data the first time someone forgets.
    """
    ...
    def __array_finalize__(self, obj):
        if obj is None:
            return
        # Copy, not share: a view or ufunc result must not be able to
        # mutate the parent frame's metadata.
        self.meta = dict(getattr(obj, "meta", None) or {})
