...
the next process()/close_resources()), so a failing decode/encode surfaces
...
class ThreadedSource(ProcessingStep):
    ...
    def _take_buffered(self, idx: int):
        """Pop frame `idx` if the prefetcher has it, dropping anything
        older. Caller must hold self._cond."""
        if idx not in self._buf:
            return None
        frame = self._buf.pop(idx)
        for older in [k for k in self._buf if k < idx]:
            del self._buf[older]
        self._cond.notify_all()
        return frame

    def _await_buffered(self, idx: int):
        with self._cond:
            while (self._running and idx not in self._buf
                   and self._exc is None):
                self._cond.wait(timeout=0.25)
            self._raise_pending()
            return self._take_buffered(idx)
    ...
    def process(self):
        ...
        with self._cond:
            frame = self._take_buffered(idx)
        if frame is not None:
            return frame

        # Not buffered. The executor already told us WHY: ctx.jumped
        # means the frame order broke, so the decoded-ahead frames belong
        # somewhere else and the prefetcher restarts here. Otherwise we
        # are simply ahead of it and just wait. This used to be inferred
        # from where idx sat relative to the prefetch window, which was a
        # guess at information the executor now supplies directly.
        if self.ctx.jumped:
            self._start_thread(start_at=idx)
        frame = self._await_buffered(idx)
        if frame is None:
            # Thread died without reporting, or the wait timed out at the
            # end of the stream: fall back to a direct read.
            self._start_thread(start_at=idx)
            frame = self._await_buffered(idx)
        if frame is None:
            raise ValueError(f"could not read frame {idx}")
...
class ThreadedSink(ProcessingStep):
    ...
    order, never concurrently):
        ...
        finalize()
            ...
            including on cancellation — same guarantee close_resources()
    ...
    def process(self, image):
        # ctx.committed is the executor's own answer to "is this for
        # real?" — a private flag mirroring it could only ever drift.
        if self.ctx.committed and self._thread is not None:
            if self._exc is not None:
                exc, self._exc = self._exc, None
                raise exc
