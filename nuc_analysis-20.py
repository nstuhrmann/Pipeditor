...
def make_plots(ds: Dataset, out: Path, pair_score, best_pair, pw_results,
    ...
    for (r_, c_), col in zip(sample, colors):
        ...
        # correction delta: mapped reference minus raw signal — the raw
        # mapping is a near-identity diagonal on which the methods are
        # visually indistinguishable; the delta isolates offset, gain tilt
        # and curvature
        # piecewise linear through its k selected calibration points
        pw_s = s_pix[pw_idx]
        srt = np.argsort(pw_s)
        ax[1].plot(dense,
                   np.interp(dense, pw_s[srt], ref[pw_idx][srt]) - dense,
                   "-", color=col, lw=1.2)
        ...
        ax[1].plot(dense, np.polyval(cf, (dense - s0_) / sc_) - dense,
                   "--", color=col, lw=1.2)
        ax[1].plot(s_pix, ref - s_pix, "o", color=col, ms=3.5)
    ax[1].axhline(0, color="gray", lw=0.6, ls=":", label="identity (Δ = 0)")
    ...
    ax[1].set(xlabel="raw pixel signal (counts)",
              ylabel="correction Δ = corrected − raw (counts)",
              title="Correction transfer curves (as delta), sample pixels")
    ax[1].legend(fontsize=8)
    fig.tight_layout()
