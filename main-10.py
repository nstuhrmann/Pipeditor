    ...
    def process_full_sequence(self):
        ...
        def on_finished(processed):
            ...
            if self._seq_worker.last_results is not None:
                ...
                # New pixmaps may differ in size from the last preview
                # frame; re-run each edge's path/thumbnail placement and
                # force a full repaint, otherwise stale thumbnail regions
                # linger (wrong position/aspect) until something else —
                # like clicking a node — happens to trigger one.
