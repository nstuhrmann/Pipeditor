...
class GatedTemporalAverage(StatefulStep):
    ...
    PARAMS = [
        ParamSpec("mode", "Filter", "choice", default="FIR",
                  choices=["FIR", "IIR"],
                  hint="FIR is the true mean of n frames (noise / sqrt(n), "
                       "n frames of memory). IIR is exponential with "
                       "alpha=1/n — one frame of memory, but it never "
                       "fully forgets, so motion smears further."),
        ...
        ParamSpec("threshold", "Contrast Threshold", "float", default=0.10,
                  ...
                  hint="Averaging runs only while contrast is below this. "
                       "Log the 'contrast' metadata on a real recording "
                       "and pick a value between its still and moving "
                       "levels."),
        ParamSpec("hysteresis", "Hysteresis", "float", default=0.0,
                  min_value=0.0, max_value=1.0, step=0.005, decimals=4,
                  hint="Widens the threshold for switching OFF only. "
                       "Stops the filter flickering when contrast sits "
                       "right at the limit — visible, because the noise "
                       "level pumps with it."),
