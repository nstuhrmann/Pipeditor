...
from src.GUI.pipeline_editor.base_step import FrameContext, STEP_REGISTRY
...
class Pipeline:
    ...
    def _context_for(self, node: PipelineNode, frame_index: int,
                     total_frames: int, committed: bool) -> FrameContext:
    ...
    def _run_frame(self, frame_index: int = 0, total_frames: int = 1,
                   on_step_done=None, committed: bool = False) -> RunResult:
        """Execute the graph once, for one frame. Internal.

        There are exactly two public ways to run a pipeline, and the
        difference between them is whether it commits:

            preview(index)        no writes, repeatable, rewinds state
            iter_sequence()       committed: opens sinks, advances state
            run_sequence()        iter_sequence() drained into one result

        `committed` is not something a caller should be choosing on a
        per-frame basis, which is why this is private."""
        result = RunResult()
        ...
        for nid in order:
            ...
            # A source has nothing upstream to pass through, so bypass
            # is meaningless there. A metric now passes its input through
            # unmodified, so bypassing one simply skips the measurement —
            # which is how you keep an expensive metric out of live
            # preview while tuning something else.
            if node.bypassed and not is_source:
                out = inputs[0]
                # Clear rather than record 0.0 ms: a bypassed node did no
                # work, so it has no measurement. Recording a zero would
                # drag its mean down and keep it in the timing table
                # claiming to be the fastest step in the graph.
                node.timing.reset()
            else:
        ...
        self._log_frame(tag, result, outbox, committed)
        return result
    ...
    def total_frames(self) -> int:
        counts = [n.step.frame_count() for n in self.nodes.values()
                  if n.step.KIND == "source"]
        counts = [c for c in counts if c > 1]
        return min(counts) if counts else 1

    def reset_state(self):
        """Drop every step's accumulated history. No I/O, so this is also
        what a seek does — it must NOT reopen sinks."""
        for node in self.nodes.values():
            node.inbox = {}
            node._last_index = None
            node.step.reset_state()

    def open_resources(self, total_frames: int):
        for node in self.nodes.values():
            node.step.open_resources(total_frames)

    def close_resources(self):
        for node in self.nodes.values():
            node.step.close_resources()

    def preview(self, frame_index: int = 0, total_frames: int = 1,
                on_step_done=None) -> RunResult:
        """Run one frame without committing: no sink writes, no resource
        lifecycle, and stateful steps can be re-run at the same index
        with changed parameters.

        Seeking needs no separate call — the executor sees the index jump
        and resets accumulated state for you."""
        return self._run_frame(frame_index=frame_index,
                               total_frames=total_frames,
                               on_step_done=on_step_done, committed=False)
    ...
    def iter_sequence(self, should_cancel=None):
        """Yield (index, total, RunResult) for each frame of a committed
        run, opening and closing resources around the whole thing.

        A generator so that every consumer shares one execution path:
        a batch drains it in a tight loop, playback pulls one frame per
        timer tick, the optimizer drains it silently. Cancellation is
        checked between frames — never mid-frame, because steps are
        stateful and interrupting one would corrupt it.
        """
        total = self.total_frames()
        self.reset_timings()
        self.open_resources(total)
        try:
            self.reset_state()
            for i in range(total):
                if should_cancel is not None and should_cancel():
                    return
                yield i, total, self._run_frame(frame_index=i,
                                                total_frames=total,
                                                committed=True)
        finally:
    ...
    def run_sequence(self, on_frame_done=None, on_progress=None,
                     should_cancel=None, workers: int = 1) -> RunResult:
        """Drain a committed run into a single RunResult.

        workers > 1 uses the parallel executor, which is verified to
        produce identical results (test_parallel_equivalence.py). The
        serial path stays the default and the reference."""
        total = self.total_frames()
        batch = RunResult()
        seen_warnings: set = set()
        if workers > 1:
            from src.GUI.pipeline_editor.parallel import (
                iter_sequence_parallel,
            )
            stream = iter_sequence_parallel(self, workers=workers,
                                            should_cancel=should_cancel)
        else:
            stream = self.iter_sequence(should_cancel)
        # No try/finally needed: both generators close their own
        # resources in a finally, including when abandoned.
        for i, total, frame in stream:
        ...
        batch.cancelled = batch.frames_processed < total
        return batch
