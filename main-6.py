...
class ImageWindow(QMainWindow):
    ...
    def show_image(self, arr: np.ndarray):
        ...
        # live-mode re-runs don't keep resetting a window the user has
        # since resized manually. Fit it to the image instead of maximizing.
        if not self._sized_once:
            self._fit_to_image(px.size())
            self._sized_once = True
        self.show()
        self.raise_()

    def _fit_to_image(self, size):
        margin_w, margin_h = 40, 80   # scrollbar / title bar allowance
        w = size.width() + margin_w
        h = size.height() + margin_h
        screen = self.screen()
        ...
        for nid, node in self.pipeline.nodes.items():
            if getattr(node.step, "IS_SINK", False) and nid in results:
                ...
                if isinstance(val, np.ndarray):
                    ...
                    self._sink_windows[nid].show_image(val)
