...
class PortItem(QGraphicsEllipseItem):
    def __init__(self, node_item: "NodeItem", is_output: bool,
        ...
        self.setBrush(QBrush(QColor(
            PORT_OUT_COLOR if is_output else PORT_IN_COLOR)))
...
class NodeItem(QGraphicsObject):
    ...
    LABEL_MARGIN = 74
    MSG_ROW_H = 11          # one line per posted topic, under the node
    ...
    def boundingRect(self) -> QRectF:
        ...
        # messages.
        extra = (len(self._bus_messages) * self.MSG_ROW_H + 4
                 if self._bus_messages else 0)
        return QRectF(-self.LABEL_MARGIN, 0,
                      NODE_WIDTH + self.LABEL_MARGIN, self.height + extra)

    def set_bus_messages(self, messages: list):
        """Replace the message lines shown under this node. Changing the
        list changes boundingRect(), so Qt must be told before it is
        re-read — hence prepareGeometryChange()."""
    ...
    def _paint_bus_messages(self, painter):
        if not self._bus_messages:
            return
        f = QFont(); f.setPointSize(6)
        fm = QFontMetrics(f)
        y = self.height + self.MSG_ROW_H - 2
        for text in self._bus_messages:
            shown = fm.elidedText(text, Qt.ElideRight,
                                  NODE_WIDTH + self.LABEL_MARGIN - 8)
            path = QPainterPath()
            path.addText(2, y, f, shown)
...
class PipelineScene(QGraphicsScene):
    ...
    def update_bus_messages(self, posts_by_node: dict):
        """Show each node's emitted messages beneath it. posts_by_node is
        MessageBus.posts_by_node — the CURRENT frame's emissions only, so
        a node that stopped posting stops showing a stale value."""
        for nid, item in self.node_items.items():
            posts = (posts_by_node or {}).get(nid) or {}
            item.set_bus_messages(
                [f"→ {topic}: {format_bus_value(v)}"
