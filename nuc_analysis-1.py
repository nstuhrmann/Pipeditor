...
class Dataset:
    ...
    navg: dict = field(default_factory=dict)       # temp -> N from filename
...
def scan_folder(folder: Path, pattern: str, raw_shape=None, raw_dtype="uint16",
    ...
    navg_seen: dict[tuple[float, float], int] = {}
    skipped = []
    for p in sorted(folder.iterdir()):
        ...
        if "navg" in m.groupdict() and m.group("navg"):
            navg_seen[(tint, temp)] = int(m.group("navg"))
    ...
    for tint in sorted({k[0] for k in groups}):
        ...
        for T in temps:
            ...
            nfr[T] = len(frames)
            if (tint, T) in navg_seen:
                nav[T] = navg_seen[(tint, T)]
        ...
        if verbose:
            ...
            navtxt = (f", N_avg per file: {sorted(set(nav.values()))}"
                      if nav else "")
            print(f"tint={tint:g} {TINT_UNIT}: {len(temps)} temps "
                  f"({temps[0]:g}..{temps[-1]:g} degC), frame {W}x{H}, "
                  f"bad pixels: {ds.bad_mask.sum()} "
                  f"({100*ds.bad_mask.mean():.3f} %){navtxt}")
    return datasets
...
def analyze_piecewise(ds: Dataset, k_max=None):
    ...
    for k in range(3, min(k_max, n) + 1):
        ...
        if comb(len(inner), k - 2) <= 3000:                  # exhaustive
            best = (None, np.inf, None)
            for c in itertools.combinations(inner, k - 2):
                idx = [0, *c, n - 1]
...
def make_plots(ds: Dataset, out: Path, pair_score, best_pair, pw_results,
    ...
    # order-d polynomial has d+1 coefficients -> comparable to (d+1)-pt pcw
    ...
    pts = "/".join(f"{temps[i]:g}" for i in r["idx"])
    decs[f"pcw_{kbest}pt"] = (residual_map_figure(
        piecewise_correct(ds, r["idx"])[w], f"{temps[w]:g} °C",
        f"pcw {kbest}-pt [{pts}] °C",
        f"{tag}_fpn_decomposition_pcw{kbest}pt.png", resp[w]), w)

    w = int(np.nanargmax(poly_results[d_best]["curve"]))
    decs[f"poly_d{d_best}"] = (residual_map_figure(
        poly_loo_frame(ds, d_best, w), f"{temps[w]:g} °C",
        f"poly d={d_best} (LOO)",
        f"{tag}_fpn_decomposition_poly_d{d_best}.png", resp[w]), w)
...
def main(argv=None):
    ...
    for tint, ds in datasets.items():
        if len(ds.temps) < 3:
            print(f"tint={tint:g} {TINT_UNIT}: <3 temperatures, skipping.")
            continue
        print(f"\n=== tint = {tint:g} {TINT_UNIT} ===")
        n = len(ds.temps)
        ...
        print("[Q2] piecewise-linear, optimal point placement:")
        for k in sorted(pw_results):
            r = pw_results[k]
        ...
        print("[Q2] polynomial, all points, leave-one-out:")
        for d in sorted(poly_results):
            print(f"      d={d}: max residual = "
        ...
        if not np.all(np.isnan(ds.temporal_std)):
            tn = to_mK(ds.temporal_std, resp)
            rep["temporal_NETD_mK"] = tn.tolist()
            print(f"[Q3] temporal NETD: {np.nanmedian(tn):.0f} mK "
                  f"(median over T; of the stored frames — already averaged "
                  f"if files are N-frame means)")
        ...
        print(f"[Q3] spatial (FPN) NETD after pcw {kbest}-pt: "
              f"median {np.nanmedian(pw_results[kbest]['curve']):.0f} mK, "
              f"max {pw_results[kbest]['score']:.0f} mK")
        print(f"[Q3] spatial (FPN) NETD after poly d={d_best_} (LOO): "
        ...
        for name, (dec, w) in decs.items():
            print(f"[+]  FPN decomposition @ worst T ({ds.temps[w]:g} °C, "
