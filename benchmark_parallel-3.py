...
def _drain(stream, total: int, label: str):
    ...
    The bar is created OUTSIDE the timed region and updated inside it —
    its own cost is a few microseconds per frame and throttled, but the
    setup is not, and a benchmark must not measure its own instrument."""
    frames = []
    with frame_progress(total, label) as bar:
