...
class PortItem(QGraphicsEllipseItem):
    LABEL_MAX_PX = 70   # on-canvas label width before eliding
    ...
    def __init__(self, node_item: "NodeItem", is_output: bool,
        ...
        if label:
            ...
            # Long names would run off the side of the node and collide
            # with incoming edges — elide on canvas, full text in the
            # tooltip (which is set above).
            shown = QFontMetrics(f).elidedText(
                label, Qt.ElideRight, self.LABEL_MAX_PX)
