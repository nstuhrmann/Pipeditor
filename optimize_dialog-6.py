...
class _OptimWorker(QObject):
    # done, total, current score, best score, best snapshot
    # (snapshot: {"best_metrics": {nid: value}, "best_values":
    #  {(nid, param): value}} — see ParameterOptimizer.best_snapshot)
    progress = Signal(int, int, float, float, object)
    ...
    def run(self):
        try:
            res = self.optimizer.run(
                on_progress=lambda n, t, cur, best, snap:
                    self.progress.emit(n, t, cur, best, snap),
...
class OptimizeDialog(QDialog):
    def __init__(self, pipeline, frame_index: int = 0,
        ...
        for nid, name in self._metrics:
            ...
            spin.setDecimals(2)
            spin.setValue(0.0)
            # Wide enough for a negative value plus the spin arrows —
            # "-1,00" was being clipped in locales that use a comma.
        ...
        tip = ("minimize: scipy Powell from the current values — the "
               "default; best for refining, and for many correlated "
               "parameters (colour matrices).\n"
               "differential_evolution: scipy's global population "
               "search — use when the current values may be far from "
               "the optimum, or several basins exist.")
        if bayes_available():
            tip += ("\ngp_minimize: Bayesian optimization "
                    "(scikit-optimize) — global and sample-efficient; "
                    "wins when one evaluation is slow (sequence mode, "
                    "large frames).")
        else:
            tip += ("\n(Install scikit-optimize to enable "
                    "'gp_minimize'.)")
        ...
        # The budget needed grows with the number of parameters — a
        # 9-coefficient colour matrix needs hundreds of evaluations,
        # while the 60 default suits one or two knobs. Say so rather
        # than letting a silently under-budgeted run look like "no
        # better solution exists".
        self.budget_hint = QLabel("")
        self.budget_hint.setWordWrap(True)
        self.budget_hint.setStyleSheet("color: #886600;")
        layout.addWidget(self.budget_hint)
        self.tree.itemChanged.connect(lambda *_: self._update_budget_hint())
        self.evals_spin.valueChanged.connect(
            lambda *_: self._update_budget_hint())
    ...
    def _update_budget_hint(self):
        ...
        if n == 0:
            ...
            return
        suggested = max(40, 25 * n)
        parts = [f"{n} parameter{'s' if n != 1 else ''} selected — "
                 f"suggest at least {suggested} evaluations."]
        if n > 4:
            parts.append("With this many parameters, narrowing each "
                         "parameter's min/max around a sensible value "
                         "helps more than any method choice.")
        self.budget_hint.setText(
            " ".join(parts) if self.evals_spin.value() < suggested else "")
    ...
    def _start(self):
        ...
        if self.method_combo.count() == 0:
            QMessageBox.information(
                self, "No optimizer available",
                "Neither scipy (minimize / differential_evolution) nor "
                "scikit-optimize (gp_minimize) is installed in this "
                "environment.")
            return
    ...
    def _on_progress(self, done, total, current, best, snapshot=None):
        self.progress.setValue(done)
        lines = [f"Evaluation {done}/{total} — score {current:.5g}, "
                 f"best {best:.5g}"]
        if snapshot:
            names = dict(self._metrics)
            for nid, v in snapshot.get("best_metrics", {}).items():
                try:
                    lines.append(f"  [{names.get(nid, nid)}] {v:.5g}")
                except (TypeError, ValueError):
                    lines.append(f"  [{names.get(nid, nid)}] {v}")
            best_values = snapshot.get("best_values", {})
            if best_values:
                parts = []
                for (nid, pname), v in best_values.items():
                    node = self.pipeline.nodes.get(nid)
                    prefix = (f"{node.display_name}." if node else "")
                    parts.append(f"{prefix}{pname}={v:.4g}"
                                 if isinstance(v, float)
                                 else f"{prefix}{pname}={v}")
                lines.append("  best: " + ", ".join(parts))
        self.status.setText("\n".join(lines))
