...
def to_uint8(arr: np.ndarray) -> np.ndarray:
    ...
    - uint16 scaled by 1/257 — 65535/255 is exactly 257, so this maps
      the full 16-bit range onto the full 8-bit range with no rounding
      drift and no clipping at either end (0->0, 65535->255)
...
def to_uint16(arr: np.ndarray) -> np.ndarray:
    ...
    if arr.dtype == np.uint16:
        return arr
    if arr.dtype == np.uint8:
        # 255 * 257 == 65535 exactly, so 8-bit values land on the exact
        # top of the 16-bit range rather than 255/65535 of it.
