...
class MessageBus:
    ...
    def __init__(self):
        self._topics: dict = {}
        # node_id -> {topic: value} emitted during the CURRENT frame.
        # Separate from _topics, which persists across frames so the
        # feedback loop works: this one is a per-frame record purely for
        # showing "what did this node just send" in the editor.
        self.posts_by_node: dict = {}

    def post(self, topic: str, value, _origin: str = None):
        self._topics[topic] = value
        if _origin is not None:
            self.posts_by_node.setdefault(_origin, {})[topic] = value
    ...
    def clear(self):
        self._topics.clear()
        self.posts_by_node.clear()

    def begin_frame(self):
        """Reset the per-frame emission record (NOT the message values —
        a command posted on frame N must still be readable on N+1)."""
        self.posts_by_node.clear()

    def view_for(self, node_id: str) -> "_BusView":
        return _BusView(self, node_id)


class _BusView:
    """Per-node handle on the bus, handed to a step as `self.bus`.

    Same API as MessageBus; the only difference is that post() records
    which node emitted the message, so the editor can display it next to
    that node without steps having to know their own id."""
    __slots__ = ("_bus", "_node_id")

    def __init__(self, bus: "MessageBus", node_id: str):
        self._bus = bus
        self._node_id = node_id

    def post(self, topic: str, value):
        self._bus.post(topic, value, _origin=self._node_id)

    def get(self, topic: str, default=None):
        return self._bus.get(topic, default)

    def pop(self, topic: str, default=None):
        return self._bus.pop(topic, default)
...
class Pipeline:
    ...
    def run(self, on_step_done=None,
        ...
        # New frame: forget which node emitted what LAST frame (the
        # message values themselves persist — that is what makes the
        # one-frame feedback delay work).
        self.bus.begin_frame()

        if metric_values_out is None and run_log.is_on("metrics"):
            metric_values_out = {}
        ...
        for nid in order:
            ...
            # A per-node view, so post() can record the sender.
            node.step.bus = self.bus.view_for(nid)
            ...
            if is_source:
                ...
                for i in range(n_inputs):
                    val, problem = self._resolve_input(
                        i, n_inputs, pred_map, results, skipped, node.step)
        ...
        if run_log.is_on("metrics") and metric_values_out:
            for mid, value in metric_values_out.items():
                name = self.nodes[mid].display_name if mid in self.nodes else mid
                run_log.log("metrics", f"{tag} metric {name}: {value}")
        if run_log.is_on("bus"):
            for sender, topics in self.bus.posts_by_node.items():
                name = (self.nodes[sender].display_name
                        if sender in self.nodes else sender)
                for topic, value in topics.items():
                    run_log.log("bus", f"{tag} bus  {name} -> {topic}: {value}")
        if run_log.is_on("warnings") and warnings_out:
    ...
    def _port_label(port_index: int, n_inputs: int, step=None) -> str:
        """Human-readable port name matching the on-canvas labels. Uses
        the step's INPUT_LABELS when it declares them, so warnings about
        asymmetric inputs name the actual role ("input Reference")
    ...
    def _resolve_input(self, port_index: int, n_inputs: int,
        ...
        if not isinstance(val, np.ndarray):
            src = self.nodes.get(from_id)
            src_name = src.display_name if src else from_id
            if src is not None and getattr(src.step, "IS_METRIC", False):
                # Shouldn't normally happen: a metric's graph output is an
                # annotated image. Kept for steps that bypass that path.
                return None, (f"{label} is connected to '{src_name}', "
                              f"which outputs a metric value, not an image.")
            return None, (
    ...
    def run_sequence(self, on_frame_done=None, on_progress=None,
        ...
        self.reset_timings()   # batch means shouldn't mix in preview runs
    ...
    def timing_summary(self) -> list:
        """[(display_name, mean_ms, total_ms, share)] sorted by total
        descending; share is the fraction of the summed step time."""
