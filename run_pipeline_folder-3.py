...
def _print_timing(rows: list, frames: int):
    """Full per-step table, on the `timing` channel. The summary only has
    room for the slowest step; this is what you turn on when that one
    number says something changed and you need to see where."""
    if not run_log.is_on("timing"):
        return
    label = "mean ms/frame" if frames > 1 else "ms"
...
def run_one(path: str, max_frames: int = 0, single: bool = False) -> Outcome:
    ...
    try:
        ...
        if rows:
            ...
            _print_timing(rows, outcome.frames)
        outcome.ok = True
