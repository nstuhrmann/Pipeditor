...
class ImageSequenceWriter(ThreadedSink):
    ...
    def write_frame(self, frame: np.ndarray):
        output_dir = self.values.get("output_dir", "")
        prefix = self.values.get("prefix", "frame")
        fmt = self.values.get("format", "PNG")
        bit_depth = self.values.get("bit_depth", "auto")
...
class VideoFileWriter(ThreadedSink):
    ...
    def process(self, image: np.ndarray, **kwargs) -> np.ndarray:
        if not self.values.get("output_path", ""):
            return image
        return super().process(image, **kwargs)

    def write_frame(self, frame: np.ndarray):
        output_path = self.values.get("output_path", "output.mp4")
        bgr = self._to_bgr_uint8(frame)
        if self._writer is None:
            h, w = bgr.shape[:2]
            fourcc = self.values.get("fourcc", "mp4v")
            fps = float(self.values.get("fps", 30.0))
