...
def step_from_class(cls, name: str = None, category: str = "General",
    ...
    if register and key in STEP_REGISTRY:
        raise ValueError(
            f"step_from_class: '{key}' is already in STEP_REGISTRY "
            f"(registered by {STEP_REGISTRY[key]!r}). Registry keys must be "
            f"unique and stable -- pass class_name= to disambiguate.")
    ...
    def create_processor(self, **params):
        kwargs = {k: v for k, v in params.items()
                  if k in self._init_param_names}
        return cls(**kwargs)

    def apply(self, processor, image, **params):
        kwargs = {k: v for k, v in params.items()
                  if k in self._call_param_names}
        return getattr(processor, call_method)(image, **kwargs)
    ...
    namespace = {
        ...
        "RESET_PARAMS": tuple(sorted(init_param_names)),
    ...
    from src.GUI.pipeline_editor.temporal_steps import _TemporalStep

    generated = type(key, (_TemporalStep,), namespace)
