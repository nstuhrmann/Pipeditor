    ...
    def measure(self) -> dict:
        ...
        try:
            if self.mode == "sequence":
                batch = self.pipeline.run_sequence(
                    should_cancel=self._should_cancel)
                for o in self.objectives:
                    out[o.node_id] = self._aggregate(
                        batch.metric_series.get(o.node_id, []))
                return out
            # A preview: never commits, and re-running the same index
            # rewinds stateful steps so each candidate is evaluated
            # against the same starting state.
            result = self.pipeline.preview(frame_index=self.frame_index,
                                           total_frames=self.total_frames)
            for o in self.objectives:
                v = result.metrics.get(o.node_id)
