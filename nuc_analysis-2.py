...
def analyze_piecewise(ds: Dataset, k_max=None):
    ...
    for k in range(3, min(k_max, n) + 1):
        ...
        if n_comb <= 3000:                                   # exhaustive
            log.info("pcw k=%d: exhaustive search over %d subsets", k, n_comb)
            ...
            for ci, c in enumerate(cand):
                ...
                if s < best[1]:
                    best = (idx, s, curve)
                _progress(ci, len(cand), f"pcw k={k} (greedy)")
            results[k] = {"idx": best[0], "score": best[1], "curve": best[2],
                          "method": "greedy"}
            current = best[0]
...
def main(argv=None):
    ...
    for tint, ds in datasets.items():
        ...
        log.info(f"[Q3] spatial (FPN) NETD after pcw {kbest}-pt: "
