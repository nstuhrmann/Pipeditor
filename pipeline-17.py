...
class Pipeline:
    ...
    def run(self, frame_index: int = 0, total_frames: int = 1,
        ...
        `committed` says this frame is part of a real run — sinks write,
        stateful steps advance for good. A preview leaves it False.
        Prefer preview() / run_sequence() over calling this directly."""
        result = RunResult()
    ...
    def preview(self, frame_index: int = 0, total_frames: int = 1,
        ...
        return self.run(frame_index=frame_index, total_frames=total_frames,
        ...
        try:
            ...
            for i in range(total):
                if should_cancel is not None and should_cancel():
                    return
                yield i, total, self.run(frame_index=i, total_frames=total,
                                         committed=True)
        finally:
