...
class FalseColor(ProcessingStep):
    ...
    def process(self, image: np.ndarray, colormap: str = "Ironbow",
        ...
        a = image
        if a.ndim == 3:
            a = (0.2126 * a[..., 0] + 0.7152 * a[..., 1]
                 + 0.0722 * a[..., 2])
        g = (np.clip(a, 0.0, 1.0) * 255.0 + 0.5).astype(np.uint8)
        if invert:
            g = 255 - g
        if colormap == "Ironbow":
...
def _match_shapes(a: np.ndarray, b: np.ndarray):
    """Promote gray to color if the two inputs disagree in channels."""
    if a.ndim == b.ndim:
        return a, b
    if a.ndim == 2:
...
class Subtract(ProcessingStep):
    ...
    def process(self, image_a: np.ndarray, image_b: np.ndarray,
                ...
                **kwargs) -> np.ndarray:
        a, b = _match_shapes(image_a, image_b)
...
class ChannelMerge(ProcessingStep):
    ...
    @staticmethod
    def _as_gray(a: np.ndarray) -> np.ndarray:
        if a.ndim == 2:
            return a
        return (0.2126 * a[..., 0] + 0.7152 * a[..., 1]
                + 0.0722 * a[..., 2]).astype(np.float32)

    def process(self, image_r: np.ndarray, image_g: np.ndarray,
                image_b: np.ndarray, **kwargs) -> np.ndarray:
        r = self._as_gray(image_r)
        g = self._as_gray(image_g)
        b = self._as_gray(image_b)
