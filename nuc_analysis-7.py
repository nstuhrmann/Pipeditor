...
def analyze_differences(ds: Dataset, out: Path) -> dict:
    ...
    for ti, T in enumerate(ds.temps):
        ...
        rows.append({"T": float(T), "resp": float(resp[ti]),
                     "Ns": [int(x) for x in Ns], **dec,
                     "column_null": col_null, "row_null": row_null,
                     "column_ratio": dec["column"] / col_null,
                     "row_ratio": dec["row"] / row_null})
        log.info("diff T=%g degC: column %.3f cts (white-null %.3f, "
                 "ratio %.1f) | row %.3f cts (null %.3f, ratio %.1f) | "
                 "pixel %.3f cts",
    ...
    ax[0].set(xlabel="BB temperature (°C)", ylabel="std of difference (mK)",
              title="Temporal noise components (difference frames)")
    ax[0].set_yscale("log")
    ...
    ax[1].legend(fontsize=8)
    # component bar chart: measured vs white-noise level per component;
    # annotated excess is quadrature-corrected (temporally correlated part)
...
def make_plots(ds: Dataset, out: Path, pair_score, best_pair, pw_results,
    ...
    def residual_map_figure(frame: np.ndarray, T_label: str, label: str,
        ...
        # difference frame (component/sqrt(2) = single-acquisition level).
        ...
        if drow is not None:
            dv = [drow[c] * to_mk / np.sqrt(2) for c in comps]
            ax[2].bar(xs + bw, dv, bw, color="tab:orange",
                      label="temporal instability (diff/√2)")
