    ...
    def _to_depth(image: np.ndarray, bit_depth: str) -> np.ndarray:
        if bit_depth == "16":
            ...
            if image.dtype == np.uint8:
                return (a * 257).astype(np.uint16)   # exact 8→16 bit scaling
            if mx > 0:
                a = a / mx * 65535
        ...
        if image.dtype == np.uint8:
            return image
        a = image.astype(np.float32) - image.min()
        mx = a.max()
        if mx > 0:
            a = a / mx * 255
...
class VideoFileWriter(ProcessingStep):
    ...
    IS_SEQUENCE_AWARE = True
    PARAMS = [
        ParamSpec("output_path", "Output Video File", "file", default="output.mp4"),
